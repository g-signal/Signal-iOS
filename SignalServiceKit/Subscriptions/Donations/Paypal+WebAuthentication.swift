...
extension Paypal {
    ...
    /// to redirect us to the `baxs://` custom scheme with all path and query
    /// components after `/redirect`.
    ///
    /// For example, the URL `https://signaldonations.org/redirect/whatever?foo=bar`
    /// will be redirected by the server to `baxs://whatever?foo=bar`.
    ...
    private static let authSessionScheme: String = "baxs"
